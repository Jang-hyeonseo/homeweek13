...
let heroImg, enemyImg, laserImg, lifeImg, explosionImg, backgroundImg, canvas, ctx, gameObjects = [], hero, eventEmitter = new EventEmitter(), gameLoopId;
...
function initGame() {
    ...
    eventEmitter.on(Messages.KEY_EVENT_SPACE, () => {
        ...
        gameObjects.filter(go => go.type === 'Hero' && go !== hero)
            .forEach(auxShip => {
                if (auxShip.canFire()) {
                    auxShip.fire();
                }
...
function moveAuxiliaryShips(xOffset, yOffset) {
    gameObjects.filter(go => go.type === 'Hero' && go !== hero)
        .forEach(auxShip => {
            auxShip.x += xOffset;
            auxShip.y += yOffset;
...
function createHero() {
    ...
    const leftWing = new Hero(hero.x - 120, hero.y);
    leftWing.img = heroImg;
    leftWing.width = 75;
    leftWing.height = 55;
    gameObjects.push(leftWing);

    const rightWing = new Hero(hero.x + 120, hero.y);
    rightWing.img = heroImg;
    rightWing.width = 75;
    rightWing.height = 55;
    gameObjects.push(rightWing);

    [leftWing, rightWing].forEach(auxShip => {
        setInterval(() => {
            if (auxShip.canFire()) {
                auxShip.fire();
            }
